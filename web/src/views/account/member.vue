    ...
    >
      ...
      >
        <template v-slot:item.id="{ item }">
          <router-link :to="{ path: '/account/member/detail/' + item.id }">{{
            item.id
          }}</router-link>
        </template>
...
export default {
  ...
  methods: {
    ...
    update(item) {
      this.$router.push("/account/member/update/" + item.id);
    },
    close() {
    ...
    save() {
      ...
      let promise;
      if ($this.item.id) {
        promise = fetch("/api/member/" + $this.item.id, {
          method: "PUT",
          headers: {
